...
# Parse the command
    ...
    parser.add_argument('-g', action='store', dest='loca', help='Location (del, mum, che, kol, hyd, ban, oth')
    ...
    parser.add_argument('-c', action='store', dest='c', help='Tweet count')
    ...
    # Location
    loca = args.loca
    if (not(loca in ('del', 'mum', 'che', 'kol', 'hyd', 'ban', 'oth')) and (loca)):
        print ("WARNING: Location must be one of these: del, mum, che, kol, hyd, ban, oth")
        exit()
    if loca:
        ge = locords[loca]
...
def tw_search(api):
    ...
    csvWriter.writerow(["author", "coordinates", "authorLoc","text", "retwc", "hashtag", "followers", "friends"])
    ...
    for tweet in tweepy.Cursor(api.search,
		...
		# Dongho 03/28/16
        if(geoenable):
            csvWriter.writerow([str(cords).encode("utf-8"), str(geoenable).encode("utf-8"), str(authorloc).encode("utf-8"), str(text).encode("utf-8"), retwc, hashtag, followers, friends])
            counter = counter +1
            if (counter == c):
                break
...
def main():
    ...
    # Delhi, Mumbai, Chennai, Kolkata, Hyderabad, Bangalore, Custom
    locords =  {'del': '77.08, 28.61, 20km',
                'mum': '72.82, 19.13, 20km',
                'che': '80.22, 13.01, 20km',
                'kol': '88.31, 22.54, 20km',
                'hyd': '78.45, 17.38, 20km',
                'ban': '77.59, 12.97, 20km',
                'oth': '0, 0, 0km'}
